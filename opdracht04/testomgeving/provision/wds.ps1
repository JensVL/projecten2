...
# try {
    ...
    $wdsUtilResults = wdsutil.exe /initialize-server /server:example.com /remInst:"E:\MDTProduction$"
    $wdsUtilResults
# } catch {
...
# try {
    ...
    Import-WdsBootImage -Path "D:\MDTProduction\Operating Systems\Windows10\sources\boot.wim"
# } catch {
    # Write-Warning "- Could not initialize the Windows Deployment Services PowerShell module"
# }
...
# try{
    ...
    Get-WindowsImage -imagePath "D:\MDTProduction\Operating Systems\Windows10\sources\install.wim" | select Imagename 
    $imageName = 'Windows 10 Pro' 
    Import-WdsInstallImage -ImageGroup "desktops" -Path "D:\MDTProduction\Operating Systems\Windows10\sources\install.wim" -ImageName $imageName
# } catch {
#     Write-Warning "- Could not initialize the Windows Deployment Services PowerShell module"
# }

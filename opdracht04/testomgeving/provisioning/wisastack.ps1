...
$service = Get-WindowsFeature AD-Domain-Services

if(!($service).Installed){
    Write-Host('Downloading and installing ADDS...')
    Install-WindowsFeature AD-Domain-Services

    Import-Module ADDSDeployment
    $safeModeAdministratorPassword = ConvertTo-SecureString 'vagrant' -AsPlainText -Force
    Install-ADDSForest -CreateDnsDelegation:$false -DatabasePath "C:\Windows\NTDS" -DomainMode "WinThreshold" -DomainName "CONTOSO" -DomainNetbiosName "CONTOSO" -ForestMode "WinThreshold" -InstallDns:$true -LogPath "C:\Windows\NTDS" -NoRebootOnCompletion:$false -SysvolPath "c:\Windows\SYSVOL" -Force:$true -SafeModeAdministratorPassword "vagrant"
} else {
